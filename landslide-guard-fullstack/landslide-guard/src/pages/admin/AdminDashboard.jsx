...
import { useRiskOverview, useAlerts, useRiskZones } from "../../api/hooks";
import { timeAgo } from "../../utils/timeAgo";

export default function AdminDashboard() {
  const [selectedZone, setSelectedZone] = useState(null);
  const { data: riskOverview, isLive: overviewLive } = useRiskOverview();
  const { data: alerts } = useAlerts();
  const { data: riskZones, isLive: zonesLive } = useRiskZones();
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-4">
          <RiskMap height="420px" onSelectZone={setSelectedZone} zones={riskZones} />
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { to: "/admin/field-reports", label: "Field Reports", desc: "Active reports" },
          ...
          {alerts.slice(0, 2).map((a) => (
            <div key={a.id} className="glass-card p-4 border-l-4" style={{ borderLeftColor: severityColor(a.severity) }}>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className="text-[10px] font-mono text-ink-mid">{a.time || timeAgo(a.createdAt)}</span>
              </div>
      ...
      <p className="text-center text-[11px] font-mono text-ink-low pt-2">
        Showing {riskZones.length} monitored zones {(overviewLive && zonesLive) ? "· Live data" : "· Demo data (backend unreachable)"}
      </p>
    </div>
  );
}
